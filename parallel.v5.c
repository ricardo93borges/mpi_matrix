    ...
    while(1) {
        ...
        if(rank == 0) {
            ...
            for(n=1; n < nprocs; n++){                
                ...
                //printf("\n Sending %d rows_to_proccess to %d", rows_to_proccess, n);
            ...
            for(n=1; n < nprocs; n++){
                ...
                //Receive rows_to_process                
                MPI_Recv(&cores, 1, MPI_INT, n, 10, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                
                //Receive rows_to_process                
                MPI_Recv(&rows_to_proccess, 1, MPI_INT, n, 9, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
                ...
                for(i=0; i < rows_to_proccess; i++){
                    for(j=0; j<SIZE; j++){
                        mres[slave_offset+i][j] = res[i][j];
                    }
                }

                if(finalize(slave_offset, rows_to_proccess, cores, nprocs, n) == 1) {
                    finalized_process[n] = n;
                }
            }
        }
        ...
        if(rank != 0){
            ...
            //Send cores to master
            MPI_Send(&cores, 1, MPI_INT, 0, 10, MPI_COMM_WORLD);
            
            //Send rows_to_process to master
            MPI_Send(&rows_to_proccess, 1, MPI_INT, 0, 9, MPI_COMM_WORLD);
