...
int main(int argc, char *argv[]) {
    ...
    int    i, j, k, n, f, rank, nprocs, name_len, offset, rows_per_proccess, cores;
    double elapsed_time;
    ...
    if(rank == 0){
        ...
        for(n=1; n < nprocs; n++){
            ...
            int o = 0;
            int k = 0;
            //printf("\n rows_per_proccess: %d", rows_per_proccess);
            //printf("\n cores: %d", cores);
            while(r < rows_per_proccess){
                //printf("\n r: %d, o: %d, k:%d", r,o,k);
                k = 0;
                for(i=o; i < (o+cores); i++){                    
                    for(j=0; j<SIZE; j++) {                        
                ...
                MPI_Send(&rows, cores*SIZE, MPI_INT, n, 5, MPI_COMM_WORLD);
                ...
                for(i=o; i < cores; i++){
                    for(j=0; j<SIZE; j++) mres[offset+i][j] = res[i][j];
                }
       ...
       /*  if(!validateMatrix()){
        ...
        int x = 0;
        int rows[cores][SIZE];
        int rows_to_multiply[cores][SIZE];
        int msres[cores][SIZE];
        omp_set_num_threads(cores);
        while(r < rows_per_proccess){
            MPI_Recv(&rows, cores*SIZE, MPI_INT, 0, 5, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
           
            for(i=0; i < cores; i++){
                for(j=0; j<SIZE; j++) rows_to_multiply[k][j] = rows[i][j];
                k++;
            }
            ...
            for (i=0 ; i<rows_per_proccess; i++) {
                for (j=0 ; j<SIZE; j++) {
                    ...
                    {
                        msres[i][j] = 0;
                        for (x=0 ; x<SIZE; x++) msres[i][j] += rows_to_multiply[i][x] * m2[x][j];
                    }
                }
            }
            ...
            //MPI_Send(&msres, cores*SIZE, MPI_INT, 0, 6, MPI_COMM_WORLD);
